...
void print_usage() {
  ...
  const char* help_info =
      "## --------------------------------------\n"
      "Calculate enrichment file using RT stop as foreground and base density "
      "as background\n\n"
      ...
      "# more options:\n"
      " -e     enrich method, subtraction or dividing or complex (default: "
      "complex)\n"
      ...
      " -g     input signals are in log rather than in normal space, then "
      "enrich method will be forced to subtraction \n\n";
...
struct Param {
  ...
  operator bool() const {
    if (input_fg_file.empty() or input_bg_file.empty() or
        output_enrichment_file.empty()) {
      cerr << RED << "Error: please specify -o, -f, and -b" << DEF << endl;
      return false;
    }

    if (winsor_factor >= 0.5) {
      cerr << RED << "Error: Winsor factor must be less than 0.5." << DEF
           << endl;
      return false;
    }
    return true;
  ...
  for (size_t i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
      if (not strcmp(argv[i] + 1, "o")) {
        ...
        cerr << RED << "FATAL ERROR: unknown option: " << argv[i] << DEF
             << endl;
        print_usage();
        exit(-1);
      }
...
void readSignal(
    const string& signalFile,
    MapStringuLONG& trans_len,
    MapStringString& trans_rpkm,
    MapStringDouble& trans_scalingFactor_bd,
    MapStringT<DoubleArray>& trans_baseDensity,
    MapStringDouble& trans_scalingFactor_rt,
  ...
  while (getline(SG, line)) {
    ...
    ++lineCount;
    if (lineCount % 10000 == 0) cerr << "\tlines " << lineCount << endl;
    ...
    if (type == "baseDensity") {
      ...
      for (auto it = data.cbegin() + 5; it != data.cend(); it++)
        baseDensities.push_back(stod(*it));
    } else if (type == "RTstop") {
      // ave_rpkm_string(rpkm, trans_rpkm[id]);
      trans_scalingFactor_rt[id] = scalingFactor;
      DoubleArray& rtStop = trans_RTstop[id];
      for (auto it = data.cbegin() + 5; it != data.cend(); it++)
        rtStop.push_back(stod(*it));
    }
  }
}

void calcEnrich(
    const MapStringuLONG& trans_bg_len,
    const MapStringT<DoubleArray>& trans_bg_baseDensity,
    const MapStringT<DoubleArray>& trans_bg_RTstop,
    const MapStringuLONG& trans_fg_len,
    const MapStringT<DoubleArray>& trans_fg_baseDensity,
    const MapStringT<DoubleArray>& trans_fg_RTstop,
  ...
  for (auto it = trans_fg_RTstop.cbegin(); it != trans_fg_RTstop.cend(); it++) {
    ...
    bool in_bg_bd =
        (trans_bg_baseDensity.find(trans) != trans_bg_baseDensity.end());
    bool in_bg_rt = (trans_bg_RTstop.find(trans) != trans_bg_RTstop.end());
    if (not in_bg_len or not in_bg_bd or not in_bg_rt) {
      cerr << RED << "Warning! transcript " << trans
           << " not defined in background file.\n"
           << DEF;
      continue;
    }
    ...
    if (not in_fg_len) {
      cerr << RED << "Warning! transcript " << trans
           << " not defined in foreground file.\n"
           << DEF;
      continue;
    }
    if (trans_fg_len.at(trans) != trans_bg_len.at(trans)) {
      cerr << RED << "Warning! transcript " << trans
           << " is of different length in background and foreground files.\n"
           << DEF;
      continue;
    }
    ...
    if (param.log_op or param.enrich_method == SUBSTRACTION) {
      ...
      for (int i = skipHead; i < trans_fg_len.at(trans) - skipTail; i++)
        if (bg_bd_array[i] > 0) {
          // char buff[100];
          // sprintf(buff, "%.3f", div_fac *
          // (fg_rt_array[i]-sub_fac*bg_rt_array[i]) / bg_bd_array[i]);
          siganlEnrichment.push_back(
              div_fac * (fg_rt_array[i] - sub_fac * bg_rt_array[i]) /
              bg_bd_array[i]);
        } else {
          siganlEnrichment.push_back(null);
        }
    }
    for (int i = trans_fg_len.at(trans) - skipTail; i < trans_fg_len.at(trans);
         i++)
      siganlEnrichment.push_back(null);
  }
}
...
void winsorWindow(
    const DoubleArray& enrichment,
    const uINT& headTopSkip,
    const uINT& trimed_last,
    const double& winsor_factor,
    double& winsorLower,
    double& winsorUpper) {
  /*
  winsor_factor /= 100;
  ...
  */
  ...
  for (uINT i = headTopSkip; i <= trimed_last; i++) {
    ...
    if (enrichment[i] != null) sortedTrimed_array.push_back(enrichment[i]);
  }
...
void winsorization(
    const MapStringuLONG& trans_len,
    MapStringT<DoubleArray>& trans_signal,
    const uINT& headTopSkip,
    const uINT& tailToSkip,
    const double& winsor_factor,
    const uINT& winsor_scaling) {
  cerr << "Winsorization...\n\t" << currentDateTime() << endl;

  uLONG transCount = 0;
  for (auto it = trans_signal.begin(); it != trans_signal.end(); it++) {
    ++transCount;
    if (transCount % 10000 == 0) cerr << "\tprocess " << transCount << "...\n";
    ...
    winsorWindow(
        ...
        winsorLower,
        winsorUpper);
    // clog << "winsorLower: " << winsorLower << "\t" << "winsorUpper: " <<
    // winsorUpper << endl;
    ...
    if (winsor_scaling != 0) {
      ...
      for (uLONG i = 0; i < len; i++) {
        if (it->second[i] != null) {
          ...
          it->second[i] = (it->second[i] - winsorLower) * winsor_scaling /
                          (winsorUpper - winsorLower);
        }
      }
    } else {
      uLONG len = trans_len.at(it->first);
...
void outputEnrichment(
    const string& outputFile,
    const MapStringuLONG& trans_len,
    const MapStringT<DoubleArray>& trans_enrichment,
    const MapStringString& trans_fg_rpkm,
    const MapStringDouble& trans_fg_scalingFactor_rt,
    const MapStringT<DoubleArray>& trans_fg_RTstop,
    const MapStringString& trans_bg_rpkm,
    const MapStringDouble& trans_bg_scalingFactor_bd,
    const MapStringDouble& trans_bg_scalingFactor_rt,
    const MapStringT<DoubleArray>& trans_bg_baseDensity,
    const MapStringT<DoubleArray>& trans_bg_RTstop) {
  ...
  for (auto it = trans_enrichment.cbegin(); it != trans_enrichment.cend();
       it++) {
    string trans(it->first);

    OUT << trans << "\t" << trans_len.at(trans) << "\t"
        << trans_fg_rpkm.at(trans) << "," << trans_bg_rpkm.at(trans) << "\t"
        << trans_fg_scalingFactor_rt.at(trans) << ","
        << trans_bg_scalingFactor_bd.at(trans) << ","
        << trans_bg_scalingFactor_rt.at(trans);
...
int main(int argc, char* argv[]) {
  ...
  readSignal(
      ...
      trans_bg_len,
      trans_bg_rpkm,
      trans_bg_scalingFactor_bd,
      trans_bg_baseDensity,
      trans_bg_scalingFactor_rt,
      trans_bg_RTstop);
  ...
  readSignal(
      ...
      trans_fg_len,
      trans_fg_rpkm,
      trans_fg_scalingFactor_bd,
      trans_fg_baseDensity,
      trans_fg_scalingFactor_rt,
      trans_fg_RTstop);
  ...
  winsorization(
      ...
      tailToSkip,
      param.winsor_factor,
      param.winsor_scaling);

  cerr << "Start to calculate outputEnrichment " << endl;
  outputEnrichment(
      param.output_enrichment_file,
      trans_fg_len,
      trans_enrichment,
      trans_fg_rpkm,
      trans_fg_scalingFactor_rt,
