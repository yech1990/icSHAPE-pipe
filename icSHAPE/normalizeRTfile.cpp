...
void print_usage() {
  ...
  const char* help_info =
      ...
      " -v     verbose mode \n"
      " -r     raw mode (100%% identical with perl icSHAPE pipeline, because I "
      "correct a bug from it)\n\n";
...
double calcScalingFactor(
    const DoubleArray& data_array,
    const uINT& start,
    const uINT& end,
    const Param& param) {
  DoubleArray sorted_data(
      data_array.cbegin() + start,
      data_array.cbegin() + end + 1);
  ...
  if (param.norm_sample_method == SMART) {
    ...
    for (auto it = sorted_data.cbegin() + selectStart;
         it != sorted_data.cbegin() + selectEnd + 1;
         it++)
...
currentDateTime() << endl;
    ...
    ofstream OUT(output_file, ofstream::out);
        ...
        if(not OUT)
    {
        cerr << RED << "FATAL Error: cannot write to " << output_file << DEF <<
endl; exit(-1);
    }
    OUT << "#transcript\tlength\ttype\tbase_frequency, start from
position 1.\n";
    ...
    for(uLONG i=0; i<contents.size(); i++)
    {
        ...
        len = stoul(data[1]);
        rpkm = data[2];
        for(auto it=data.cbegin()+3; it!=data.cend(); it++)
baseDensity.push_back( stod(*it) );
        ...
        while(trimed_last < headToSkip+40)
        {
            ...
            cerr << "Warning! Transcript $transcript too short. update
headToSkip: " << headToSkip << "; tailToSkip: " << tailToSkip << endl;
            if(headToSkip==0 and tailToSkip==0) break;
        }

        scalling_factor = calcScalingFactor(baseDensity, headToSkip,
trimed_last, param);

        if(scalling_factor > 1)
        {
            scalling_factor = scalling_factor / scalling_form;
            OUT << transcript << "\t" << len << "\tbaseDensity\t" << rpkm <<
"\t" << scalling_factor; if(norm_method == "log")
            {
                for(int i=1; i<=len; i++)
                {
                    char buff[400];
                    sprintf(buff, "%.3f",
log2(baseDensity[i]/scalling_factor+1)); OUT << "\t" << buff;
                }
            }else{
                for(int i=1; i<=len; i++)
                {
        ...
        //trim(line);
        //split(line, '\t', data);
        for(auto it=rt_data.cbegin()+3; it!=rt_data.cend(); it++)
rtstop.push_back( stod(*it) ); scalling_factor = calcScalingFactor(rtstop,
headToSkip, trimed_last, param);

        if(scalling_factor > 1)
        {
            scalling_factor = scalling_factor / scalling_form;

            OUT << transcript << "\t" << len << "\tRTstop\t" << rpkm << "\t" <<
scalling_factor; if(norm_method == "log")
            {
                for(int i=1; i<=len; i++)
                {
...
void normalizeBaseDensity(const Param& param) {
  ...
  while (getline(IN, line)) {
    ...
    ++lineCount;
    if (lineCount % 1000 == 0) cerr << "  line: " << lineCount << endl;
    ...
    while (trimed_last < headToSkip + 40) {
      ...
      if (not param.verbose)
        cerr << "Warning! Transcript $transcript too short. update headToSkip: "
             << headToSkip << "; tailToSkip: " << tailToSkip << endl;
      if (headToSkip == 0 and tailToSkip == 0) break;
    }

    scalling_factor =
        calcScalingFactor(baseDensity, headToSkip, trimed_last, param);
    ...
    if (scalling_factor > 1) {
      ...
      if (param.verbose)
        cerr << "Filter RTstop of " << transcript
             << " for small scalling_factor=" << scalling_factor << endl;
    }
  }
