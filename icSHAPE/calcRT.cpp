...
void readRPKM(
    const string& rpkmFile,
    const double& minLoad,
...
void calcBaseDensity(
  ...
  while (getline(IN, line)) {
    ...
    ++lineCount;
    // cout << lineCount << endl;
    if (lineCount % 1000000 == 0) cerr << "\tlines " << lineCount << endl;
...
void output_baseDensity(
    const string& outputFile,
    const MapStringuLONG& trans_len,
    const MapStringDouble& trans_rpkm,
    const MapStringT<uLONGArray>& baseDensity,
    const MapStringT<uLONGArray>& RTstop) {
