...
void print_usage() {
  ...
  const char *help_info =
      ...
      "\e[1mHELP:\e[0m\n"
      "\t-in: input a sam or bam file, the file extension should be .sam or "
      ".bam\n"
      "\t-out: output a tab-seperated file \n"
      "\t-sort: <yes/no> whether to sort the output file (default: yes)\n"
      "\t-ruj: <yes/no> whether to remove reads with unannotated junctions, "
      "only useful for .sam file (default: yes)\n\n"
  ...
  sprintf(buff, help_info, warning.str().c_str(), BINVERSION, LIBVERSION, DATE,
          "Li Pan");
  cerr << buff << endl;
}
...
bool Sort_Map_Record(const Map_Record *const mr_1,
                     const Map_Record *const mr_2) {
...
void sam2tab(const Param &param) {
  ...
  if (endswith(param.input_file, ".bam")) {
    ...
    if (not bam_hd) {
      cerr << RED << "FATAL Error: open file " << param.input_file << " failed"
           << DEF << endl;
      exit(-1);
    }
    ...
    if (not IN) {
      cerr << RED << "FATAL Error: cannot read " << param.input_file << DEF
           << endl;
      exit(-1);
    }
  } else {
    cerr << RED << "FATAL Error: input file must be sam or bam file -- "
         << param.input_file << DEF << endl;
    exit(-1);
  }
  ...
  if (success and sam_head.trans_len.size() >= 1) {
    ...
    for (auto it = sam_head.trans_len.cbegin(); it != sam_head.trans_len.cend();
         it++) {
  ...
  while (1) {
    ...
    mapChrRecords.content[read_record.chr_id + read_record.strand()]
        ->content.push_back(new Map_Record(
            matchRegion, read_record.strand() == '+' ? POS : NEG));
  }
  ...
  if (param.rem_anno_junc) {
    ...
    cerr << "remove " << removed_unanno << " reads with unannotated junctions"
         << endl;
  }

  if (param.will_sort) {
    cerr << "Start to sort..." << endl;
    for (auto it = mapChrRecords.content.begin();
         it != mapChrRecords.content.end(); it++) {
      const string &chr_id = it->first;
      sort(it->second->content.begin(), it->second->content.end(),
           Sort_Map_Record);
    }
  }
  ...
  if (not OUT) {
    cerr << RED << "FATAL Error: cannot write " << param.output_file << DEF
         << endl;
    exit(-1);
  }
  ...
  for (auto it = chr_list.begin(); it != chr_list.end(); it++) {
    ...
    for (auto it2 = records->content.cbegin(); it2 != records->content.cend();
         it2++) {
      OUT << chr_id << '\t' << strand;
