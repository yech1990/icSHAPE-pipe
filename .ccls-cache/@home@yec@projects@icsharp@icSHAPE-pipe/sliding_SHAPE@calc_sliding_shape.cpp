...
void print_usage_root() {
  ...
  const char *help_info =
      ...
      "\e[1mHELP:\e[0m\n"
      "\t[mode]\n"
      "\t\tTrtCont: calculate SHAPE score with Treatment (NAI-N3) and Control "
      "(DMSO) samples \n"
...
void print_usage_smartSHAPE() {
  char buff[3000];
  const char *help_info =
      "calc_sliding_shape Trt - calculate SHAPE score with a sliding window "
      "with Treatment (NAI-N3) samples\n"
      "========================================================================"
      "============================\n"
      "\e[1mUSAGE:\e[0m\n"
      "\tsliding_shape Trt -N inputTab1,inputTab2... -size chrSizeFile [-ijf "
      "junctionFile] -out out.gTab\n"
      "\e[1mHELP:\e[0m\n"
      "\t-N: input Treatment (NAI-N3) tab files generated by sam2tab (must be "
      "sorted)\n"
      "\t-size: input a chromosome size file generated by STAR "
      "(chrNameLength.txt) \n"
      "\t-ijf: input a junction file generated by STAR "
      ...
      "\t-enm: <rt/div/log> enrich method. RT means use raw RT to calculate "
      "SHAPE;\n"
      "\t      div means use RT/BD to calculate SHAPE;\n"
      "\t      log means use log2(RT+add_fac) to calculate SHAPE (default: "
      "rt)\n"
      ...
      "\t-bases: base to calculate shape scores, such as A,C,a,c, -genome "
      "should be provided\n"
      "\t-non-sliding: Calculate the shape score as a whole, without using the "
      "sliding window (default: None)\n"
      "\t              Suitable for short RNAs. Some parameters will be "
      "ignored in this case\n"
      "\t-separate: Each base is calculated separately (default: None) "
      "-genome/-bases should be provided\n\n"
...
void print_usage_icSHAPE() {
  char buff[3000];
  const char *help_info =
      "calc_sliding_shape TrtCont - calculate SHAPE score with a sliding "
      "window with Treatment (NAI-N3) and Control (DMSO) samples\n"
      "========================================================================"
      "===================================================\n"
      "\e[1mUSAGE:\e[0m\n"
      "\tsliding_shape TrtCont -D inputTab1,inputTab2... -N "
      "inputTab1,inputTab2... -size chrSizeFile [-ijf junctionFile] -out "
      "out.gTab\n"
      "\e[1mHELP:\e[0m\n"
      "\t-D: input Control (DMSO) tab files generated by sam2tab (must be "
      "sorted)\n"
      "\t-N: input Treatment (NAI-N3) tab files generated by sam2tab (must be "
      "sorted)\n"
      "\t-size: input a chromosome size file generated by STAR "
      ...
      "\t-nocm: <mean/median/peak> calculate a scalling factor from sampled "
      "data.(default: median)\n"
      "\t\t mean: mean value \n"
      "\t\t median: median value \n"
      "\t\t peak: max value \n"
...
struct General_Param {
  ...
  operator bool() {
    ...
    if (not bases.empty() and genome_seq_file.empty()) {
      cerr << RED
           << "FATAL Error: -genome should be provided when -bases specified"
           << DEF << endl;
      return false;
    }
    ...
    if (base_separate and (bases.empty() or genome_seq_file.empty())) {
      cerr << RED
           << "FATAL Error: -bases and -genome should be provided when "
              "-separate specified"
           << DEF << endl;
      return false;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (param.mode == TrtCont) {
    ...
    while (sync_chrs(i_vec, chr_ids, record_array)) {
      ...
      if (use_mask and not fasta.has_chr(chr)) {
        cerr << RED << "Warning: " << chr_id
             << " not found on genome file, skip it" << DEF << endl;
        continue;
      }

      if (chr_size.find(chr_id) == chr_size.end()) {
        cerr << RED << "Warning: " << chr_id
             << " not found on size file, skip it" << DEF << endl;
        continue;
      }

      // 2. calculation
      clog << index++ << ". read_chr " << chr_id
           << "\tsite: " << chr_size.at(chr_id) << endl;

      if (junctions.find(chr_id) == junctions.end()) {
        if (not param.no_sliding)
          cerr << RED << "Warning: " << chr_id
               << " junction not found, init empty junctions" << DEF << endl;
        junctions[chr_id];
      }
      ...
      if (strand == '+') {
        ...
        for (uLONG d = 0; d < D_num; d++) {
          calc_chr_BDRT_Pos(d_bd, d_rt, record_array.at(d), junctions[chr_id],
                            cSize, BD_ext, binsize);
          // for(int i=0;l)
        }
        for (uLONG n = 0; n < N_num; n++)
          calc_chr_BDRT_Pos(n_bd, n_rt, record_array.at(n + D_num),
                            junctions[chr_id], cSize, BD_ext, binsize);

        // clog << "Start to sliding_non_junction..." << endl;
        // sliding_non_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id],
        // score, chr, POSITIVE, OUT, shape_param, chr_mask, chr_seq);

        // clog << "Start to sliding_junction..." << endl;
        // sliding_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score,
        // chr, POSITIVE, OUT, shape_param, chr_mask, chr_seq);
      } else {
        clog << "Start to calc_chr_BDRT_Neg" << endl;
        for (uLONG d = 0; d < D_num; d++)
          calc_chr_BDRT_Neg(d_bd, d_rt, record_array.at(d), junctions[chr_id],
                            cSize, BD_ext, binsize);
        for (uLONG n = 0; n < N_num; n++)
          calc_chr_BDRT_Neg(n_bd, n_rt, record_array.at(n + D_num),
                            junctions[chr_id], cSize, BD_ext, binsize);

        // clog << "Start to sliding_non_junction..." << endl;
        // sliding_non_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id],
        // score, chr, NEGATIVE, OUT, shape_param, chr_mask, chr_seq);

        // clog << "Start to sliding_junction..." << endl;
        // sliding_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score,
        // chr, NEGATIVE, OUT, shape_param, chr_mask, chr_seq);
      }
      ...
      if (param.base_separate) {
        for (char c : param.bases) {
          ...
          sliding_non_junction(n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score,
                               chr, s, OUT, shape_param, chr_mask, chr_seq);
          sliding_junction(n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score,
                           chr, s, OUT, shape_param, chr_mask, chr_seq);
        }
      } else {
        ...
        if (use_mask)
          bool success =
              build_chr_mask(fasta, chr, s, param.bases, chr_mask, chr_seq);
        sliding_non_junction(n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score,
                             chr, s, OUT, shape_param, chr_mask, chr_seq);
        sliding_junction(n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score, chr,
                         s, OUT, shape_param, chr_mask, chr_seq);
      }
      //}
      ////////////////////////////////////////////////////////////////////////
    ...
    while (sync_chrs(i_vec, chr_ids, record_array)) {
      ...
      // 2. calculation
      clog << index++ << ". read_chr " << chr_id << "\t" << chr_size.at(chr_id)
           << endl;

      if (junctions.find(chr_id) == junctions.end()) {
        if (not param.no_sliding)
          cerr << RED << "Warning: " << chr_id
               << " junction not found, init a empty junction" << DEF << endl;
        junctions[chr_id];
      }
      ...
      if (strand == '+') {
        clog << "Start to calc_chr_BDRT_Pos" << endl;
        for (uLONG n = 0; n < N_num; n++)
          calc_chr_BDRT_Pos(n_bd, n_rt, record_array.at(n), junctions[chr_id],
                            cSize, shape_param.BD_ext, binsize);

        // clog << "Start to sliding_non_junction..." << endl;
        // sliding_non_junction( n_rt, n_bd, junctions[chr_id], score, chr,
        // POSITIVE, OUT, shape_param, chr_mask, chr_seq);

        // clog << "Start to sliding_junction..." << endl;
        // sliding_junction( n_rt, n_bd, junctions[chr_id], score, chr,
        // POSITIVE, OUT, shape_param, chr_mask, chr_seq);
      } else {

        clog << "Start to calc_chr_BDRT_Pos" << endl;
        for (uLONG n = 0; n < N_num; n++)
          calc_chr_BDRT_Neg(n_bd, n_rt, record_array.at(n), junctions[chr_id],
                            cSize, shape_param.BD_ext, binsize);

        // clog << "Start to sliding_non_junction..." << endl;
        // sliding_non_junction( n_rt, n_bd, junctions[chr_id], score, chr,
        // NEGATIVE, OUT, shape_param, chr_mask, chr_seq);
      ...
      if (param.base_separate) {
        for (char c : param.bases) {
          ...
          if (use_mask)
            bool success = build_chr_mask(fasta, chr, s, v, chr_mask, chr_seq);
          sliding_non_junction(n_rt, n_bd, junctions[chr_id], score, chr, s,
                               OUT, shape_param, chr_mask, chr_seq);
          sliding_junction(n_rt, n_bd, junctions[chr_id], score, chr, s, OUT,
                           shape_param, chr_mask, chr_seq);
        }
      } else {
        clog << "Start to calculate all base " << param.bases << endl;
        if (use_mask)
          bool success =
              build_chr_mask(fasta, chr, s, param.bases, chr_mask, chr_seq);
        sliding_non_junction(n_rt, n_bd, junctions[chr_id], score, chr, s, OUT,
                             shape_param, chr_mask, chr_seq);
        sliding_junction(n_rt, n_bd, junctions[chr_id], score, chr, s, OUT,
                         shape_param, chr_mask, chr_seq);
      }
