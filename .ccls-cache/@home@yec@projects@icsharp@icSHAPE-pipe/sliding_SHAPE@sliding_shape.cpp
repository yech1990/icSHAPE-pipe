...
void load_junctions(const string &file_name,
                    MapStringT<JunctionArray> &junctions,
                    const MapStringuLONG &chr_size) {
  junctions.clear();
  ...
  while (getline(IN, line)) {
    ...
    if (chr_size.find(chr_id) != chr_size.end()) {
      if (chr_size.at(chr_id) > e)
        ...
        cerr << RED << "Warning: " << line
             << " in juntion file exceed the length of chromosome "
             << chr_size.at(chr_id) << " , skip it" << DEF << endl;
    } else {
      cerr << RED << "Warning: " << chr_id
           << " in juntion file not in size file, skip it" << DEF << endl;
    }
  }
  IN.close();
...
// build junction index, left means build with junction start; right means build
// with junction end
void buildLeftJunctionMap(JunctionArray &junctions,
...
void buildRightJunctionMap(JunctionArray &junctions,
                           map<uLONG, vector<Junction *>> &junc_map,
                           const uLONG &binsize) {
  junc_map.clear();
  for (auto it = junctions.begin(); it != junctions.end(); it++) {
...
void build_junction_support(const vector<Map_Record> &record_array,
  ...
  for (auto it = record_array.cbegin(); it != record_array.cend(); it++) {
    for (auto it2 = it->regions.cbegin(); it2 != it->regions.cend() - 1;
         it2++) {
...
void check_overlap(JunctionArray &junctions, const uLONG &size) {
  for (uLONG i = 0; i < junctions.size() - 1;) {
    ...
    if (ne > size + 1) {
      cerr << RED << "FATAL Error: junction exceed chromsome length(" << size
           << "): " << ns << "-" << ne << "; remove it" << DEF << endl;
    ...
    if (cs <= ne + 1 and ns <= ce + 1) {
      ...
      junctions.erase(junctions.begin() + i + 1);
      cerr << RED << "Warning: combine overlap: " << cs << "-" << ce << ";"
           << ns << "-" << ne << DEF << endl;
    }
...
void write_junctions(const MapStringT<JunctionArray> &junctions,
  ...
  for (auto it = junctions.cbegin(); it != junctions.cend(); it++) {
    ...
    for (const Junction &ja : it->second)
      OUT << chr << "\t" << strand << "\t" << ja.first << "\t" << ja.second
          << "\t" << ja.support << "\n";
  }
...
bool read_chr(vector<Map_Record> &record_array, ifstream *hander,
              string &chr_id) {
  if (hander->eof())
    return false;
...
// load chromosome size file, it can be chrNameLength.txt from STAR index
// directory
...
// calculate chromsome RT and BD in positive strand (input record_array must be
// in positive strand)
void calc_chr_BDRT_Pos(uIntArray &BD, uIntArray &RT,
                       const vector<Map_Record> &record_array,
                       JunctionArray &junctions, const uLONG &size,
                       const uLONG &BD_ext, const uLONG &binsize) {
  // build junction map
  map<uLONG, vector<Junction *>> junc_map;
        ...
        for (uINT i = 0; i < cur_junc_map.size(); i++) {
          ...
          if (cur_jp->first <= s and s <= cur_jp->second) {
            ...
            if (i != cur_junc_map.size() - 1)
              ...
              nex_jp =
                  new Junction((bin_id + 1) * binsize, (bin_id + 1) * binsize);
...
// calculate chromsome RT and BD in negative strand (input record_array must be
// in negative strand)
              ...
              while (ext_left > 0) {
                if (end == cur_pos) {
                  if (cur_jun_index < cur_junc_map.size() - 1) {
                    ...
                    cur_pos = cur_junc_map[cur_jun_index - 1]->second +
                              1; // cur_jp->first-1;
                  } else {
...
// calculate averaged score from a float array, if the valid score number is
// less than half of number, it will return null
...
// discriminate if a region is low expressed, is the number of base exceed minBD
// exceed bd.size()/5, then it is not a low exp region
...
bool valid_cov(const deque<float> &bd, const deque<float> &rt,
               const float &min_bd, const float &min_rt) {
  uLONG big_bd_num =
      count_if(bd.cbegin(), bd.cend(),
               [&min_bd](const float &v) { return v >= min_bd; });
...
void sliding_non_junction(const uIntArray &NAI_RT, const uIntArray &DMSO_RT,
  ...
  if (use_mask and NAI_RT.size() != chr_mask.size())
    throw runtime_error("NAI_RT.size() != chr_mask.size() -- " +
                        to_string(NAI_RT.size()) +
                        " != " + to_string(chr_mask.size()));
  ...
  if (DMSO_RT.size() != chr_size + 1 or NAI_BD.size() != chr_size + 1 or
      DMSO_BD.size() != chr_size + 1) {
  ...
  if (param.no_sliding) {
    ...
    if (chr_size < 50) {
      cerr << RED << "Warning: Chromosome " << chr_id << "(" << strand_char
           << ")"
           << " is too short. Skip it" << chr_size;
      return;
    }
    ...
    if (use_mask)
      calculate_mask_score(nai_rt, nai_bd, dmso_rt, dmso_bd, scores, mask,
                           param);
    else
    ...
    for (uLONG index = 1; index < chr_size; index++) {
      ...
      if (masked and (NAI_RT[index] >= 1 or DMSO_RT[index] >= 1 or
                      DMSO_BD[index] >= out_min_cov)) {
        OUT << chr_id << '\t' << strand_char << '\t' << index << '\t' << base
            << NAI_RT[index] << '\t' << NAI_BD[index] << '\t' << DMSO_RT[index]
            << '\t' << DMSO_BD[index] << '\t' << score[index] << '\t'
            << (score[index] == null ? 0 : 1) << '\t' << score[index] << '\n';
      }
    }
    return;
  }
  ...
  if (chr_size < wsize * 2) {
    cerr << RED << "Warning: Chromosome " << chr_id << "(" << strand_char << ")"
         << " is too short. Skip it" << chr_size;
    cerr << "Not use sliding window (-non-sliding) or use a shorter window "
            "size (-wsize) is recommend for this one"
         << DEF << endl;
    return;
  }
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  while (cur_i <= chr_size) {
    for (long i = 0; i < wstep and cur_i <= chr_size; i++) {
      if (cur_i % 1000000 == 0)
        clog << "\t lines: " << cur_i << "(" << 100.0 * cur_i / chr_size
             << "%) wStep: " << wstep << endl;
  ...
  if (has_junction)
    if (ji <= junctions.size() - 1)
      cerr << RED << "Error: Junction index not reach final "
           << junctions.at(ji).first << "-" << junctions.at(ji).second << DEF
           << endl;
...
void sliding_junction(const uIntArray &NAI_RT, const uIntArray &DMSO_RT,
                      const uIntArray &NAI_BD, const uIntArray &DMSO_BD,
                      const JunctionArray &junctions, FloatArray &score,
                      const string &chr_id, const STRAND &strand, ofstream &OUT,
                      const icSHAPE_Param &param, const vector<bool> &chr_mask,
  ...
  for (const Junction &junc : junctions) {
    ...
    sliding_single_junction(NAI_RT, DMSO_RT, NAI_BD, DMSO_BD, junc.first,
                            junc.second, score, chr_id, strand, OUT, param,
                            chr_mask, chr_seq);
  }
}
...
void sliding_single_junction(const uIntArray &NAI_RT, const uIntArray &DMSO_RT,
                             ...
                             FloatArray &score, const string &chr_id,
                             const STRAND &strand, ofstream &OUT,
                             const icSHAPE_Param &param,
                             const vector<bool> &chr_mask,
                             const string &chr_seq) {
  bool use_mask = chr_mask.empty() ? false : true;
  if (use_mask and NAI_RT.size() != chr_mask.size())
    throw runtime_error("NAI_RT.size() != chr_mask.size() -- " +
      ...
      if (index >= start and index <= end) {
        ...
        if (masked and (NAI_RT[index] >= 1 or DMSO_RT[index] >= 1 or
          ...
          OUT << chr_id << '\t' << strand_char << '\t' << index << '\t' << base
              ...
              << DMSO_RT[index] << '\t' << DMSO_BD[index] << '\t' << shape_score
              << '\t' << valid_count << '\t';
...
void calculate_mask_score(const deque<float> &nai_rt,
                          const deque<float> &nai_bd,
                          const deque<float> &dmso_rt,
                          const deque<float> &dmso_bd, FloatArray &scores,
                          const deque<bool> &mask, const icSHAPE_Param &param) {
  scores.clear();
  ...
  */
  ...
  calcEnrich(dmso_bd_mask, dmso_rt_mask, nai_bd_mask, nai_rt_mask, dmso_bd_sf,
             dmso_rt_sf, nai_bd_sf, nai_rt_sf, scores_mask, param);
...
void calcEnrich(const deque<float> &dmso_bd, const deque<float> &dmso_rt,
  ...
  if (param.enrich_method == SUBSTRACTION) {
    for (uLONG i = 0; i < length; i++)
      score.push_back((nai_rt[i] / nai_rt_sf) -
                      sub_fac * (dmso_rt[i] / dmso_rt_sf));
  } else if (param.enrich_method == DIVIDING) {
    for (uLONG i = 0; i < length; i++)
      if (dmso_bd[i] > 0)
        score.push_back(div_fac * (nai_bd[i] / nai_bd_sf) /
                        (dmso_bd[i] / dmso_bd_sf));
      else
    ...
    for (uLONG i = 0; i < length; i++)
      if (dmso_bd[i] > 0)
        score.push_back(
            div_fac *
            ((nai_rt[i] / nai_rt_sf) - sub_fac * (dmso_rt[i] / dmso_rt_sf)) /
            (dmso_bd[i] / dmso_bd_sf));
      else
        score.push_back(null);
    ...
    for (uLONG i = 0; i < length; i++)
      if (dmso_bd[i] > 0) {
        ...
        // cout << "(nai_rt[i]+add_fac)/(dmso_rt[i]+add_fac): " <<
        // (nai_rt[i]+add_fac)/(dmso_rt[i]+add_fac) << endl;
...
void sliding_non_junction(const uIntArray &NAI_RT, const uIntArray &NAI_BD,
  ...
  if (param.no_sliding) {
    ...
    for (uLONG index = 1; index < chr_size; index++) {
      ...
      if (masked and (NAI_RT[index] >= 1 or NAI_BD[index] >= out_min_cov)) {
        OUT << chr_id << '\t' << strand_char << '\t' << index << '\t' << base
            << NAI_RT[index] << '\t' << NAI_BD[index] << '\t' << score[index]
            << '\t' << (score[index] == null ? 0 : 1)
            << '\t'
  ...
  while (cur_i <= chr_size) {
    for (long i = 0; i < wstep and cur_i <= chr_size; i++) {
      ...
      if (masked and (NAI_RT[index] >= 1 or NAI_BD[index] >= out_min_cov)) {
        OUT << chr_id << '\t' << strand_char << '\t' << index << '\t' << base
            << NAI_RT[index] << '\t' << NAI_BD[index] << '\t' << shape_score
            << '\t' << valid_count << '\t';
...
void sliding_junction(const uIntArray &NAI_RT, const uIntArray &NAI_BD,
  ...
  for (const Junction &junc : junctions) {
    if (++index % 100 == 0)
      clog << "\t junc ratio: " << 100.0 * index / junc_num << "%" << endl;
    sliding_single_junction(NAI_RT, NAI_BD, junc.first, junc.second, score,
                            chr_id, strand, OUT, param, chr_mask, chr_seq);
  }
}
...
void calculate_mask_score(const deque<float> &nai_rt,
                          const deque<float> &nai_bd, FloatArray &scores,
                          const deque<bool> &mask,
                          const smartSHAPE_Param &param) {
  scores.clear();
  ...
  */
  ...
  if (nai_bd_mask.size() != nai_bd_mask.size()) {
    cerr << RED << "Error: nai_bd_mask.size() != nai_bd_mask.size()" << DEF
         << endl;
    exit(-1);
  }
...
// calculate smart-SHAPE score with NAI RT and BD
void calculate_score(const deque<float> &nai_rt, const deque<float> &nai_bd,
                     FloatArray &scores, const smartSHAPE_Param &param) {
  scores.clear();
...
void calcEnrich(const deque<float> &nai_bd, const deque<float> &nai_rt,
                FloatArray &score, const smartSHAPE_Param &param) {
  // float sub_fac = param.sub_factor;
  // float div_fac = param.div_factor;
  float add_fac = param.add_factor;
...
void winsorWindow(const FloatArray &score, const float &winsor_factor,
                  float &winsorLower, float &winsorUpper) {
...
float calcScalingFactor(const deque<float> &data_array,
                        const icSHAPE_Param &param) {
  ...
  if (param.norm_sample_method == SMART) {
    ...
    for (auto it = sorted_data.cbegin() + selectStart;
         it != sorted_data.cbegin() + selectEnd + 1; it++) {
  ...
  if (param.norm_calc_method == MEDIAN) {
    float median = 1;
    if (len % 2 == 0) {
      median = (rangeOfSelection[len / 2 - 1] + rangeOfSelection[len / 2]) / 2;
    } else {
      median = rangeOfSelection[(len - 1) / 2];
    }
    scalling_factor = median;

  } else if (param.norm_calc_method == MEAN) {
    float Sum = 0.0;
    for (const float &v : rangeOfSelection)
      Sum += v;
...
bool sync_chrs(vector<ifstream *> &i_vec, StringArray &chr_ids,
               vector<vector<Map_Record>> &record_array) {
