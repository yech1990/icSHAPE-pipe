...
void print_usage() {
  ...
  const char *help_info =
      ...
      "=============================================================\n"
      "\e[1mUSAGE:\e[0m\n"
      "\tcountRT -in input1.tab,input2.tab... -size chrSizeFile [-ijf "
      "junctionFile] -out outputFile\n"
      "\e[1mHELP:\e[0m\n"
...
void write_RTBD(ofstream &OUT, const string &chr_id_strand,
                vector<uIntArray> &rt_Array, vector<uIntArray> &bd_Array,
                const string &chr_seq, const uINT min_D) {
  const uLONG cSize = bd_Array.front().size() - 1;

  if (rt_Array.size() != bd_Array.size()) {
    cerr << RED << "FATAL Error: rt_Array and bd_Array different length" << DEF
         << endl;
    exit(-1);
  }
  if (not chr_seq.empty() and rt_Array.front().size() != chr_seq.size() + 1) {
    cerr << RED << "FATAL Error: rt_Array and chr_seq different length "
         << rt_Array.front().size() - 1 << " and " << chr_seq.size()
         << ". Skip this one" << DEF << endl;
    return;
  }
  ...
  for (uLONG i = 1; i <= cSize; i++) {
    ...
    if (any_of(rt_Array.begin(), rt_Array.end(),
               [&i](uIntArray &a) { return a[i] >= 1; }) or
        any_of(bd_Array.begin(), bd_Array.end(),
               [&i, &min_D](uIntArray &a) { return a[i] >= min_D; })) {
...
bool fetch_chr_seq(qFasta &seq_holder, const string &chrID,
                   const STRAND &strand, string &chr_seq) {
  ...
  */
  ...
  while (sync_chrs(i_vec, chr_ids, record_array)) {
    ...
    if (chr_size.find(chr_id) == chr_size.end()) {
      cerr << RED << "Warning: " << chr_id << " not found on size file" << DEF
           << endl;
      continue;
    }

    if (junctions.find(chr_id) == junctions.end()) {
      cerr << RED << "Warning: " << chr_id << " junction not found" << DEF
           << endl;
      junctions[chr_id];
    }
    ...
    if (record_array.front().front().strand == POSITIVE) {
      ...
      for (uLONG i = 0; i < i_vec.size(); i++) {
        uLONG BD_ext = 0;
        calc_chr_BDRT_Pos(bd_array.at(i), rt_array.at(i), record_array.at(i),
      ...
      for (uLONG i = 0; i < i_vec.size(); i++) {
        uLONG BD_ext = 0;
        calc_chr_BDRT_Neg(bd_array.at(i), rt_array.at(i), record_array.at(i),
                          junctions[chr_id], cSize, BD_ext, binsize);
      }
    }
