...
def init(
    ...
    return ParseTrans.ParseTransClass(
        genomeCoorBedFile,
        seqFileName=seqFn,
        showAttr=showAttr,
        remove_tid_version=rem_tVersion,
        remove_gid_version=rem_gVersion,
    )
...
def initGTF(
    ...
    if source == "Gencode" or source == "Ensembl":
        handle = GTFParserFunc.read_ensembl_gtf(
            AnnotationGTF, rem_scaffold=rem_scaffold
        )
        Parser = __build_parser(
            ...
            source=source,
            showAttr=showAttr,
            rem_tVersion=rem_tVersion,
            rem_gVersion=rem_gVersion,
            verbose=verbose,
        )
    elif source == "NCBI":
        handle = GTFParserFunc.read_ncbi_gff3(
            AnnotationGTF, rem_scaffold=rem_scaffold, raw_chrID=raw_chrID
        )
        Parser = __build_parser(
            handle,
            genomeFn=genomeFile,
...
def __build_parser(
    ...
    if source == "Gencode" or source == "Ensembl":
        GTFParserFunc.write_gtf_genomeCoor_bed(
            GenomeHandler, tmp_genomeCoor_file
        )
    elif source == "NCBI":
        GTFParserFunc.write_gff3_genomeCoor_bed(
            GenomeHandler, tmp_genomeCoor_file, raw_chrID=raw_chrID
        )
    else:
    ...
    if genomeFn:
        ...
        GTFParserFunc.writeTranscriptome(
            tmp_genomeCoor_file,
            genomeFn,
            tmp_transcriptome_file,
            verbose=verbose,
    ...
    Parser = ParseTrans.ParseTransClass(
        tmp_genomeCoor_file,
        seqFileName=tmp_transcriptome_file,
        showAttr=showAttr,
        remove_tid_version=rem_tVersion,
        remove_gid_version=rem_gVersion,
    )
