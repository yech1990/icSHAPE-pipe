...
___ParseTrans = {"first_biuldGeneParser": 1}
...
def format_Exon_UTR_str(raw_str, strand):
    regions_list = [
        (int(region.split("-")[0]), int(region.split("-")[1]))
        for region in raw_str.split(",")
    ]
    ...
    return ",".join(
        [str(region[0]) + "-" + str(region[1]) for region in regions_list]
    )
...
def parseUTR(utr_str, trans_len):
    ...
    elem_coor = {
        "utr_5_start": utr_5_start,
        "utr_5_end": utr_5_end,
        "utr_3_start": utr_3_start,
        "utr_3_end": utr_3_end,
        "cds_start": cds_start,
        "cds_end": cds_end,
    }
...
def ParseGenomeCoorBedFileLine(
    GAPer, Line, remove_tid_version=False, remove_gid_version=False
):
    arr = Line.strip().split("\t")
    (Chr, Start, End, Strand, GeneName_GeneID, TransID, GeneType, Exon) = arr[
        :8
    ]

    Exon = format_Exon_UTR_str(Exon, Strand)
    (GeneName, GeneID) = (
        GeneName_GeneID.split("=")[0],
        GeneName_GeneID.split("=")[1],
    )

    if remove_tid_version and "." in TransID:
    ...
    if len(arr) == 9:
        ...
        utrString = ",".join(
            [str(item[0]) + "-" + str(item[1]) for item in utrList]
        )
...
def ParseGenomeCoorBedFile(
    genomeCoorBedFile,
    showAttr=True,
    remove_tid_version=False,
    remove_gid_version=False,
):
    """Get genome-annotation parser."""
    GAPer = dict()

    for line in open(genomeCoorBedFile):
        ParseGenomeCoorBedFileLine(
            GAPer,
            line,
            remove_tid_version=remove_tid_version,
            remove_gid_version=remove_gid_version,
        )
    ...
    if showAttr and len(GAPer) != 0:
        ...
        showBeautifulExample(
            exmaple, title="Object Example (%s)" % (example_tid,)
        )
...
def biuldGeneParser(GAPer, showAttr=True):
    if ___ParseTrans["first_biuldGeneParser"]:
        sys.stderr.writelines(
            "Warning: Gene Coordinate System: [1_based_start, 1_based_end]\n"
        )
    ...
    for transID in GAPer:
        ...
        try:
            ...
            if geneType not in geneParser[geneID]["gene_type"]:
                geneParser[geneID]["gene_type"].append(geneType)
            if Start < geneParser[geneID]["start"]:
                geneParser[geneID]["start"] = Start
                geneParser[geneID]["length"] = (
                    geneParser[geneID]["end"] - geneParser[geneID]["start"] + 1
                )
    ...
    if showAttr and len(geneParser) != 0:
        ...
        showBeautifulExample(
            exmaple, title="Object Example (%s)" % (exmaple_gid,)
        )
...
def GeneIntron(GAPer, geneParser, geneID):
    ...
    def Intron_From_Exom(exon_str, start, end, strand):
        ...
        genomeCoorIntron = []
        exonList = [
            [int(exon_region.split("-")[0]), int(exon_region.split("-")[1])]
            for exon_region in exon_str.split(",")
        ]
        if len(exonList) == 0:
            raise NoExonError
        ...
        for idx in range(len(exonList) - 1):
            ...
            genomeCoorIntron.append(
                [exonList[idx][1] + 1, exonList[idx + 1][0] - 1]
            )
        if end > exonList[-1][1]:
        ...
        if strand == "+":
            ...
            for idx in range(len(GeneCoorIntron)):
                ...
                GeneCoorIntron[idx][0], GeneCoorIntron[idx][1] = (
                    GeneCoorIntron[idx][1],
                    GeneCoorIntron[idx][0],
                )
        genomeCoorIntron.sort(
            key=lambda x: x[0], reverse=True if strand == "-" else False
        )
    ...
    for transID in geneParser[geneID]["transcript"]:
        ...
        try:
            ...
            IsoformIntrons[transID] = Intron_From_Exom(
                exon_str, trans_start, trans_end, strand
            )
        except NoExonError:
            sys.stderr.writelines(
                "Warning: %s have no Exon_str, Skip it\n" % (transID,)
            )
...
def GeneExon(GAPer, geneParser, geneID):
    ...
    def Exom_from_str(exon_str, start, end, strand):
        ...
        for idx in range(len(exonList)):
            if strand == "+":
                gene_start, gene_end = (
                    exonList[idx][0] - start + 1,
                    exonList[idx][1] - start + 1,
                )
            elif strand == "-":
                gene_end, gene_start = (
                    end - exonList[idx][0] + 1,
                    end - exonList[idx][1] + 1,
                )
    ...
    for transID in geneParser[geneID]["transcript"]:
        exon_str = GAPer[transID]["exon_str"]
        try:
            IsoformExons[transID] = Exom_from_str(
                exon_str, gene_start, gene_end, strand
            )
        except NoExonError:
            sys.stderr.writelines(
                "Warning: %s have no Exon_str, Skip it\n" % (transID,)
...
def eval_start_stop_codon(GAPer, Sequence):
    ...
    for tid in GAPer:
        ...
        if cds_start > 10 and cds_end + 10 < trans_len:
            ...
            if start_3 == "ATG":
                start_plus_3 += 1
            if stop_1 in stop_list:
                stop_minus_3 += 1
            if stop_2 in stop_list:
                stop_0 += 1
            if stop_3 in stop_list:
                stop_plus_3 += 1
    ...
    if start_0 < start_minus_3 or start_0 < start_plus_3:
        sys.stderr.writelines(
            "Unexpected Error: start codon must be the first 3 bases in CDS\n"
        )
...
def showRNAStructure(transFeature, sequence, bias=0):
    ...
    if len(CDS) % 3 != 0:
        sys.stderr.writelines(
            "Warning: The length of the CDS is not an integer multiple of 3\n"
        )
    ...
    while i < len(CDS):
        if j % 2 == 0:
            formatCDS += Colors.f(
                CDS[i : i + 3], fc="black", bc="yellow", ft="normal"
            )
        else:
            formatCDS += Colors.f(
                CDS[i : i + 3], fc="black", bc="lightyellow", ft="normal"
            )
        i += 3
        j += 1
...
def getRNAPosition(Parser, tid, region):
    """return type: [ "not_mRNA", "5UTR", "span_5UTR_CDS", "CDS",
    "span_CDS_3UTR", "3UTR", "span_5UTR_CDS_3UTR", "INVALID" ]"""
    ...
    if e < cds_start:
        return "5UTR"
    if s < cds_start <= e <= cds_end:
        return "span_5UTR_CDS"
    if cds_start <= s <= e <= cds_end:
        return "CDS"
    if cds_start <= s <= cds_end < e:
        return "span_CDS_3UTR"
    if cds_end < s:
        return "3UTR"
    if s < cds_start < cds_end < e:
        return "span_5UTR_CDS_3UTR"
    return "INVALID"
...
class ParseTransClass(object):
    def __init__(
        ...
        self.GAPer = ParseGenomeCoorBedFile(
            self.genomeCoorBedFile,
            showAttr=showAttr,
            remove_tid_version=remove_tid_version,
            remove_gid_version=remove_gid_version,
        )
        if self.seqFileName != "":
            self.Seq = readSeq(
                self.seqFileName, remove_tid_version=remove_tid_version
            )
            self.stop_bias = eval_start_stop_codon(self.GAPer, self.Seq)

        sys.stderr.writelines(
            "Warning: Trans/Gene Coorninate System: [1_based_start, 1_based_end]\n"
        )
    ...
    def getTransFeature(self, transID, showAttr=False, verbose=True):
        ...
        try:
            ...
            if verbose:
                sys.stderr.writelines(
                    "Warning: no this transID: %s in GAPer\n" % (transID,)
                )
            raise KeyError

        if showAttr:
            showBeautifulExample(
                transFeature, title="%s information" % (transID,)
            )
    ...
    def genomeCoor2transCoor(self, chrID, start, end, strand):
        ...
        return CoorFunc.genomeCoor2transCoor(
            TransBin, self, chrID, start, end, strand
        )
    ...
    def getmRNAGeneList(self):
        ...
        mRNAGeneSet = set(
            [self.GAPer[tid]["gene_id"] for tid in mRNATransList]
        )
    ...
    def getGeneByGeneName(self, geneName):
        ...
        if not hasattr(self, "GeneNameToGeneID"):
            ...
            self.GeneNameToGeneID = {
                GeneParser[gid]["gene_name"]: gid for gid in GeneParser
            }
