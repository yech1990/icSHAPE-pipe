...
def format_UTR(exonString, cdsString, strand, verbose=True):
    ...
    if strand == "+":
        for (exon_start, exon_end) in exonList:
            ...
            elif (exon_start < cdsList[0][0] and exon_end == cdsList[0][1]) or (
                exon_start < cdsList[0][0] and len(cdsList) == 1
            ):
        ...
        for (exon_start, exon_end) in exonList[::-1]:
            ...
            elif (cdsList[-1][1] < exon_end and exon_start == cdsList[-1][0]) or (
                cdsList[-1][1] < exon_end and len(cdsList) == 1
            ):
        ...
        for (exon_start, exon_end) in exonList:
            if exon_start > cdsList[0][1]:
                utr_5.append((exon_start, exon_end))
            elif (cdsList[0][1] < exon_end and exon_start == cdsList[0][0]) or (
                cdsList[0][1] < exon_end and len(cdsList) == 1
            ):
        ...
        for (exon_start, exon_end) in exonList[::-1]:
            if exon_end < cdsList[-1][0]:
                utr_3.append((exon_start, exon_end))
            elif (exon_start < cdsList[-1][0] and exon_end == cdsList[-1][1]) or (
                exon_start < cdsList[-1][0] and len(cdsList) == 1
            ):
...
def writeTranscriptome(
    genomeCoorFileName,
    genomeFaFile,
    outTranscriptomeFaFile,
    verbose=True,
    showAttr=False,
):
    import GAP
        ...
        for tid in geneParser[geneID]["transcript"]:
                ...
                if ChrID not in no_chr_set:
                    ...
                    if verbose:
                        sys.stderr.writelines(
                            "Warning: %s not in %s" % (ChrID, genomeFaFile) + "\n"
                        )
                continue

            exonsList = Parser.getGeneExon(geneID)[tid][0]
            if strand == "-":
                exonsList.sort(key=lambda x: x[0], reverse=True)
            ...
            for exon in exonsList:
                try:
                    ...
                    if verbose:
                        sys.stderr.writelines(
                            "Warning: KeyError -> %s\t%d\t%d\t%s"
                            % (ChrID, exon[0] - 1, exon[1], strand)
                            + "\n"
                        )
                    continue
...
def read_ensembl_gtf(gtfFileName, rem_scaffold=False):
    ...
    for line in open(gtfFileName):
        ...
        attributes["chr"] = Chr
        attributes["start"] = Chr_Start
        attributes["end"] = Chr_End
        ...
        if Type == "gene":
            ...
            if "gene_name" not in attributes:
                attributes["gene_name"] = "NULL"
            if (
                "transcript_biotype" not in attributes
                and "transcript_type" not in attributes
            ):
            ...
            gtf_container["exon"][attributes["transcript_id"]] = gtf_container[
                "exon"
            ].get(attributes["transcript_id"], []) + [attributes]
        elif Type == "CDS":
            gtf_container["CDS"][attributes["transcript_id"]] = gtf_container[
                "CDS"
            ].get(attributes["transcript_id"], []) + [attributes]
...
def write_gtf_genomeCoor_bed(gtf_container, genomeCoorFileName):
    ...
    return_code, output = getstatusoutput(
        "sort -k 1,1 -k2n,3n %s > %s" % (tmpFileName, genomeCoorFileName)
    )
    if return_code != 0:
        print(
            "Error: cat and sort step is broken. Please check the file: " + tmpFileName
        )
    else:
...
def get_format_Ensembl_gtf_RNA_item(gtf_container, rna_ID, verbose=True):
    ...
    if rna_ID in gtf_container["CDS"]:
        ...
        try:
            (utr_5_list, utr_3_list) = format_UTR(
                exonString, cdsString, Strand, verbose=verbose
            )
...
def read_ncbi_gff3(gff3FileName, rem_scaffold=False, raw_chrID=False):
    ...
    for line in open(gff3FileName):
        ...
        if Type == "region":
            ...
            gff3_container["exon"][RNA_id] = gff3_container["exon"].get(RNA_id, []) + [
                attributes
            ]
            RNA_ids.add(RNA_id)
            ...
            RNA_id = attributes["Parent"]
            gff3_container["CDS"][RNA_id] = gff3_container["CDS"].get(RNA_id, []) + [
                attributes
            ]
            mRNA_ids.add(RNA_id)
...
def write_gff3_genomeCoor_bed(gff3_container, genomeCoorFileName, raw_chrID=False):
    ...
    for rna_ID in gff3_container["RNA"]:
        transItem = get_format_NCBI_gff3_RNA_item(
            gff3_container, rna_ID, NCToChr, rnaIDToTransID
        )
        if transItem:
            string = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
                transItem[0],
...
def get_format_NCBI_gff3_RNA_item(
    gff3_container, rna_ID, NCToChr, rnaIDTotransID, verbose=True
):
    """
...
def norm_exons(string):
    ...
    for indx in range(len(arr)):
        aarr = arr[indx].split("-")
        if indx == 0:
            tuples.append((1, abs(int(aarr[1]) - int(aarr[0])) + 1))
        else:
            tuples.append(
                (
                    tuples[-1][1] + 1,
                    abs(int(aarr[1]) - int(aarr[0])) + tuples[-1][1] + 1,
                )
            )
        ...
        for i in range(len(raw_tuple)):
            if strand == "-":
                if raw_tuple[i][0] == aarr[0] <= aarr[1] == raw_tuple[i][1]:
                    utr_tuple.append((norm_tuple[i][0], norm_tuple[i][1]))
                    flag = True
                    break
                elif raw_tuple[i][0] == aarr[0] <= aarr[1] < raw_tuple[i][1]:
                    utr_tuple.append(
                        (raw_tuple[i][1] - aarr[1] + norm_tuple[i][0], norm_tuple[i][1])
                    )
                    flag = True
                    break
                elif raw_tuple[i][0] < aarr[0] <= aarr[1] == raw_tuple[i][1]:
                    utr_tuple.append(
                        (norm_tuple[i][0], raw_tuple[i][1] - aarr[0] + norm_tuple[i][0])
                    )
                    flag = True
                    break
            if strand == "+":
                if raw_tuple[i][0] == aarr[0] <= aarr[1] == raw_tuple[i][1]:
                    ...
                    utr_tuple.append(
                        (norm_tuple[i][0], norm_tuple[i][0] + aarr[1] - aarr[0])
                    )
                    flag = True
                    break
                elif raw_tuple[i][0] < aarr[0] <= aarr[1] == raw_tuple[i][1]:
                    utr_tuple.append(
                        (norm_tuple[i][1] - (aarr[1] - aarr[0]), norm_tuple[i][1])
                    )
                    flag = True
                    break
        if not flag:
...
def genomeCoorBed_To_transCoorBed(genomeCoorFileName, transCoorFileName):
    ...
    while line:
        ...
        transCoorString = (
            arr[5] + "\t" + arr[4] + "\t" + arr[6] + "\t" + str(Len) + "\t" + exonString
        )
        if len(arr) == 9:
