...
def exonstr2exonlist(exon_str):
    ...
    exon_pair = [
        [int(it.split("-")[0]), int(it.split("-")[1])] for it in exon_items
    ]
    return exon_pair
...
def genomeRange2TransCoor(
    TransBin, Parser, Chr, Start, End, Strand, bw=100000
):
    """左闭右开，0-based."""
    idxBin = int(Start / bw)
    overlapRegions = []
    ...
    while idxBin <= int(End / bw):
        ...
        for trans_id in TransBin[Chr][Strand][idxBin]:
            ...
            overlapRegion = overlapTrans(
                trans_id, exonArray, Strand, Start, End
            )
            for item in overlapRegion:
                item.insert(0, Chr)
    ...
    for idxExon in range(numExon):
        ...
        if exonStart <= absEnd and exonEnd >= absStart:
            ...
            if strand == "+":
                ...
                if endPosInExon > exonLength:
                    endPosInExon = exonLength
            elif strand == "-":
            ...
            overlapRegions.append(
                [absStartInExon, absEndInExon, transID, relStart, relEnd]
            )
        relExonStart += exonLength
    return overlapRegions


def genomeCoor2transCoor(
    TransBin, Parser, Chr, Start, End, Strand, pureTransID=False, bw=100000
):
    # covert 1-based to 0-based
    assert Start <= End
    if Start <= 0:
        raise out_of_range()
    raw_transCoorList = genomeRange2TransCoor(
        TransBin,
        Parser,
        Chr=Chr,
        Start=Start - 1,
    ...
    for (
        ...
        trans_start,
        trans_end,
    ) in raw_transCoorList:
        if pureTransID:
            rnaID = rnaID.strip().split(".")[0]
        if chr_start <= chr_end:
            clean_transCoorList.append(
                [Chr, chr_start, chr_end, rnaID, trans_start, trans_end]
            )
...
def transCoor2genomeCoor(Parser, TransID, Pos):
    ...
    if Pos > Parser.getTransFeature(TransID)["trans_len"]:
        raise out_of_range()
    ...
    sys.stderr.writelines(
        "Warning: %s %s cannot be found genome Coordinate\n" % (TransID, Pos)
    )
...
def geneCoor2genomeCoor(Parser, GeneID, Pos):
    ...
    (Chr, Chr_Start, Chr_End, Strand, GeneName) = (
        Gene["chr"],
        Gene["start"],
        Gene["end"],
        Gene["strand"],
...
def overlapGene(geneParser, geneID, strand, absStart, absEnd):
    ...
    if absStart <= geneEnd and geneStart <= absEnd:
        if strand == "+":
            ...
            if endPosInGene > geneLength:
                endPosInGene = geneLength
        elif strand == "-":
...
def genomeCoor2geneCoor(
    GeneBin, Parser, Chr, Start, End, Strand, pureGeneID=False, bw=100000
):
    # covert 1-based to 0-based
    assert Start <= End
    if Start <= 0:
        raise out_of_range()
    raw_geneCoorList = genomeRange2geneCoor(
        GeneBin,
        Parser,
        Chr=Chr,
        Start=Start - 1,
...
def geneCoor2transCoor(TransBin, Parser, GeneID, Start, End):
    ...
    if Start <= 0:
        raise out_of_range()
    (genome_chr, genome_pos_start, genome_strand) = geneCoor2genomeCoor(
        Parser, GeneID, Start
    )
    (genome_chr, genome_pos_end, genome_strand) = geneCoor2genomeCoor(
        Parser, GeneID, End
    )
    if genome_chr == False:
        return []
...
def transCoor2geneCoor(GeneBin, Parser, TransID, Start, End):
    ...
    if Start <= 0:
        raise out_of_range()
    (genome_chr, genome_pos_start, genome_strand) = transCoor2genomeCoor(
        Parser, TransID, Start
    )
    (genome_chr, genome_pos_end, genome_strand) = transCoor2genomeCoor(
        Parser, TransID, End
    )
    if genome_chr == False:
        return []
