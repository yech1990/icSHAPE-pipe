...
def init():
    ...
    opts, args = getopt.getopt(
        sys.argv[1:], "hg:o:s:", ["genome=", "noscaffold", "rawchr"]
    )
...
def main():
    ...
    if params["source"] in ("gencode", "ensembl"):
        gtf_container = GTFParserFunc.read_ensembl_gtf(
            params["input"], rem_scaffold=params["noscaffold"]
        )
        GTFParserFunc.write_gtf_genomeCoor_bed(gtf_container, genomeCoorFn)
    elif params["source"] == "ncbi":
        gff3_container = GTFParserFunc.read_ncbi_gff3(
            params["input"],
            rem_scaffold=params["noscaffold"],
            raw_chrID=params["rawchr"],
        )
        GTFParserFunc.write_gff3_genomeCoor_bed(
            gff3_container, genomeCoorFn, raw_chrID=params["rawchr"]
        )
