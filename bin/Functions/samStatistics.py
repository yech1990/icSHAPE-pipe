    ...
    for alignSeg in IN:
        ...
        if cur_query_name == last_query_id:
            sys.stderr.writelines(
                "Error: Each reads should be output for one time (--outSAMmultNmax 1)"
                + "\n"
            )
            exit(-1)
...
Exon_list = list(Exon.items())
Exon_list.sort(key=lambda x: x[1], reverse=True)
Intron_list = list(Intron.items())
Intron_list.sort(key=lambda x: x[1], reverse=True)
...
for map_type, count in Intron_list:
    intron_row += html_table_row_temp % (
        map_type,
        "{:,}".format(int(count)),
        100.0 * count / total,
    )
...
OUT.writelines(
    html.format(pure_file, command, basic_info, map_row, exon_row, intron_row)
)
OUT.close()
