...
def PlotTransSHAPEStatistics(SHAPE, Parser, outPDF):
    ...
    objs1 = pie(
        list(df_trans_element["count"]),
        colors=list(df_trans_element["color"]),
        labels=list(df_trans_element["label"]),
        textColorFollows=True,
        labeldistance=1.01,
    )
    ...
    table1 = plt.table(
        cellText=df_trans_element.loc[:, ("gtype", "count")].values,
        loc="center",
        cellLoc="left",
        colLabels=["geneType", "count"],
    ...
    objs2 = pie(
        list(df_base_element["count"]),
        colors=list(df_base_element["color"]),
        labels=list(df_base_element["label"]),
        textColorFollows=True,
        labeldistance=1.01,
    )
    ...
    table2 = plt.table(
        cellText=df_base_element.loc[:, ("gtype", "count")].values,
        loc="center",
        cellLoc="left",
        colLabels=["geneType", "count"],
        cellColours=cellColors,
...
def genetype_gini(SHAPE, Parser):
    ...
    for tid in SHAPE:
        ...
        if gt == "mRNA":
            ...
            if gini_5:
                GINI["UTR5"].append(gini_5)
            if gini_cds:
                GINI["CDS"].append(gini_cds)
            if gini_3:
                GINI["UTR3"].append(gini_3)
