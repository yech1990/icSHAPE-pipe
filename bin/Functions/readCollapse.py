...
def init():
    ...
    if "inFile" in Params:
        ...
        if (
            ("inFile_1" not in Params)
            or ("inFile_2" not in Params)
            or ("outFile_1" not in Params)
            or ("outFile_2" not in Params)
        ):
            sys.stderr.writelines(
                "Error: \n\tSigle-end mode: specify -U, -o\n\tPair-end mode: specify -1, -2, -p and -q"
                + "\n"
...
def main():
    ...
    if Params["mode"] == "single":
        ...
        if os.path.exists(Params["outFile"]):
            sys.stderr.writelines(
                "Warning! %s exisits, will be overwritten. \n\t" % (Params["outFile"],)
                + "\n"
            )
    ...
    if cLen > 0:
        sys.stderr.writelines(
            "File %s too large, will be splitted..." % (inFile,) + "\n"
        )
        file2Collapse.pop()
        tmpOutDir = tmpDir
        CMD = SplitBarcode + " -i %s -o %s -p %s -l %s --mode new" % (
            inFile,
            tmpOutDir,
            cPos,
            cLen,
    ...
    for file in file2Collapse:
        ...
        if Params["outFasta"] != None:
            CMD = collapseFq + " -i %s -o %s --mode append --fasta %s" % (
                file,
                outFile,
                Params["outFasta"],
            )
    ...
    if Params["mode"] == "pair":
        ...
        sys.stderr.writelines(
            "Collapsing file %s and %s finished."
            % (Params["inFile_1"], Params["inFile_2"])
            + "\n"
        )
    else:
        sys.stderr.writelines(
            "Collapsing file %s finished." % (Params["inFile"],) + "\n"
        )
...
def mergePairEndReads(readFile1, readFile2, peFile):
    ...
    if readFile1.endswith(".gz"):
        ...
        sys.stderr.writelines(
            getoutput("gzip -d -c %s > %s" % (readFile1, inFile1)) + "\n"
        )
    elif readFile2.endswith(".gz"):
        inFile2 = readFile2 + ".tmp.fq"
        sys.stderr.writelines(
            getoutput("gzip -d -c %s > %s" % (readFile2, inFile2)) + "\n"
        )

    sys.stderr.writelines(
        getoutput("paste %s %s > %s" % (inFile1, inFile2, peFile)) + "\n"
    )
...
def splitPairEndReads(peFile, readFile1, readFile2):
    ...
    R1 = (
        gzip.open(readFile1, "wt")
        if readFile1.endswith(".gz")
        else open(readFile1, "w")
    )
    R2 = (
        gzip.open(readFile2, "wt")
        if readFile2.endswith(".gz")
        else open(readFile2, "w")
    )
    for line in PE:
        r1, r2 = line.split("\t")
        R1.writelines(r1)
