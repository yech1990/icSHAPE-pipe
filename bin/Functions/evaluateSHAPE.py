...
def init():
    params = {
        ...
        "accessFn": None,
        "minArea": 5.0,
        "ignore_double_strand": False,
    }
    opts, args = getopt.getopt(
        sys.argv[1:],
        "hi:s:o:",
        ["step=", "accessiblity=", "min_area=", "ignore_double_strand"],
    )
...
def filter_seq_shape(
    shape,
    dot,
    params,
    accessibility=None,
    aligned_shape_seq=None,
    aligned_access_seq=None,
):
...
def main():
    ...
    for tid in common_tid:
        ...
        assert len(shape) == len(
            dot
        ), f"structure length {len(dot)} != shape length {len(shape)} in {tid}"
        ...
        if tid in Access:
            ...
            aligned_shape_seq, aligned_access_seq = Structure.multi_alignment(
                [shape_seq, Access_Fasta[tid]]
            )
            ...
            new_shape, new_dot = filter_seq_shape(
                aligned_shape,
                aligned_dot,
                params,
                accessibility=aligned_access,
                aligned_shape_seq=aligned_shape_seq,
                aligned_access_seq=aligned_access_seq,
            )
        else:
        ...
        print(
            f"{tid} AUC: {AUC:.3}; {pair_num} paired bases {unpair_num} unpaired bases"
        )
