...
def init():
    ...
    if ("annotation" in params) and ("sizeFile" in params):
        sys.stderr.writelines(
            "Error:  -s and -g cannot be specified at the same time" + "\n"
        )
        exit(-1)
...
def genomeSHAPE2TransSHAPE(chr_id, strand, genomeSHAPEList, Parser, outFile):
    ...
    for pos, shape, RT in genomeSHAPEList:
        ...
        if len(trans_location_list) == 0:
            try:
                gene_location_list = Parser.genomeCoor2geneCoor(
                    chr_id, pos, pos, strand
                )
            except:
                continue
    ...
    OUT.writelines(
        "#%s\t%s\t%s\t%s" % (chr_id + strand, exon, intergenic, intron) + "\n"
    )
...
def filterSHAPE(
    ...
    for tid in inputSHAPE:
        ...
        RT_list = [
            int(it[1]) for it in inputSHAPE[tid][leadingNULL:end] if it[1] != "NULL"
        ]

        valid_shape_len = len(shape_list[leadingNULL:end]) - shape_list[
            leadingNULL:end
        ].count("NULL")
...
def parallel_genomeSHAPE2TransSHAPE(
    inFile,
    Parser,
    minBD=100,
    threads=5,
    ...
    while len(process_list) < threads:
        chr_id, chr_strand, genomeSHAPEList = read_A_ChrStrand(
            IN, gTab_head, minBD=minBD
        )
        if not chr_id:
            break
        ...
        p = Process(
            target=genomeSHAPE2TransSHAPE,
            args=(chr_id, chr_strand, genomeSHAPEList, Parser, tmpFile),
        )
        p.start()
    ...
    while len(process_list) != 0:
        ...
        filtSHAPE = filterSHAPE(
            rawSHAPE,
            minCovRatio=minCovRatio,
            minCovNum=minCovNum,
            minAveRT=minAveRT,
            minFPKM=minFPKM,
            FPKMDict=FPKMDict,
    ...
    for chr_id, exon, intergenic, intron in countList:
        print(
            ...
            % (
                ...
                intergenic,
                intron,
                100.0 * exon / (exon + intergenic + intron + 1),
            )
        )
    ...
    print(
        ...
        % (
            ...
            t_intergentic,
            t_intron,
            100.0 * t_exon / (t_exon + t_intergentic + t_intron),
        )
    )
...
def genomeSHAPEToGenomeSHAPE(inFile, ChrSize, minBD):
    ...
    cur_chr_id, cur_chr_strand, genomeSHAPEList = read_A_ChrStrand(
        IN, gTab_head, minBD=minBD
    )
    while cur_chr_id:
        if cur_chr_strand == "+":
...
def main():
    ...
    if "annotation" in params:
        ...
        transSHAPE = parallel_genomeSHAPE2TransSHAPE(
            params["input"],
            GAPer,
            minBD=params["minBD"],
            threads=params["threads"],
            minCovRatio=params["minCovRatio"],
            minCovNum=params["minCovNum"],
            minAveRT=params["minAveRT"],
            minFPKM=params["minFPKM"],
            FPKMDict=fpkm,
        )
        saveTransSHAPE(transSHAPE, params["output"], params["append"], FPKMDict=fpkm)
    else:
        ChrSize = load_chr_size(params["sizeFile"])
        genomeSHAPE = genomeSHAPEToGenomeSHAPE(
            params["input"], ChrSize, minBD=params["minBD"]
        )
        filt_genomeSHAPE = filterSHAPE(
            genomeSHAPE,
            minCovRatio=params["minCovRatio"],
            minCovNum=params["minCovNum"],
            minAveRT=params["minAveRT"],
            minFPKM=params["minFPKM"],
            FPKMDict=fpkm,
            leadingNULL=params["leadingNULL"],
            tailingNULL=params["tailingNULL"],
        )
        saveTransSHAPE(
            filt_genomeSHAPE, params["output"], params["append"], FPKMDict=fpkm
        )
