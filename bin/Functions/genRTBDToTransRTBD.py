...
def init():
    ...
    if ("annotation" in params) and ("sizeFile" in params):
        sys.stderr.writelines(
            "Error: -s and -g cannot be specified simultaneously" + "\n"
        )
        print(Usage)
        sys.exit(-1)
    return params
...
def parallel_genomeSHAPETotransSHAPE(genomeSHAPE, Parser, final_outFile, threads=5):
    ...
    while len(chr_left) > 0:
        ...
        while len(chr_left) > 0 and threads_left > 0:
            ...
            p = Process(
                target=genomeSHAPETotransSHAPE,
                args=(genomeSHAPE[cur_chr], Parser, outFile),
            )
...
def main():
    ...
    if "sizeFile" not in parameters:
        ...
        parallel_genomeSHAPETotransSHAPE(
            genomeSHAPE, Parser, parameters["output"], threads=parameters["threads"]
        )
