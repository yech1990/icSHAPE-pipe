...
def main():
    ...
    totalCount, representativeCount, collapseRatio, fastaFile = collapse(
        Params["inFile"],
        Params["outFile"],
        Params["mode"],
        Params["outFasta"],
    ...
    sys.stderr.writelines(
        "Read collapse successful! Total count: %s, unique count: %s, unique ratio: %s. \nRefer to the collapsed fasta file: %s"
        % (totalCount, representativeCount, collapseRatio, fastaFile)
        + "\n"
    )
...
def collapse(inFile, outFile, mode="new", fastaFile=None, simplify=False):
    ...
    if mode == "new":
        OUT1 = (
            gzip.open(outFile, "wt") if outFile.endswith(".gz") else open(outFile, "w")
        )
        FA = (
            gzip.open(fastaFile, "wt")
            if fastaFile.endswith(".gz")
            else open(fastaFile, "w")
        )
    elif mode == "append":
        OUT1 = (
            gzip.open(outFile, "at") if outFile.endswith(".gz") else open(outFile, "a")
        )
        FA = (
            gzip.open(fastaFile, "at")
            if fastaFile.endswith(".gz")
            else open(fastaFile, "a")
        )
