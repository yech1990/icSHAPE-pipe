...
def main():
    ...
    lib_outFile, bc_count = splitByLibrary(
        Params["inFile"],
        Params["outDir"],
        inFileName,
        inFileSuffix,
    ...
    sys.stderr.writelines(
        "File %s successfully splited into:" % (Params["inFile"],) + "\n"
    )
...
def splitByLibrary(
    ...
    lib_outFile = {}
    prepareLibraryOutputFiles(
        outDir, fileName, fileSuffix, lib_outFile, lib_info, library, mode, compress
    )
...
def prepareLibraryOutputFiles(
    ...
    if mode == "new" and os.path.exists(lib_outFile["unmatched"]):
        sys.stderr.writelines(
            "Warning! %s exists...will be erased\n" % (lib_outFile["unmatched"],) + "\n"
        )
    ...
    for barcode in lib_info:
        ...
        if mode == "new":
            name = (
                (lib + "_" + fileName) if (library != None) else (fileName + "_" + lib)
            )
...
def outputStat(statFile, bc_count, lib_info, inFile, library=None, mode="new"):
    if mode == "new":
        if os.path.exists(statFile):
            sys.stderr.writelines(
                "Warning! %s exists and will be overwitten.\n" % (statFile,) + "\n"
            )
