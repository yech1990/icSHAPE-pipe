            ...
            for b in params["bases"]:
                if len(b) != 1:
                    sys.stderr.writelines(
                        "Error: --bases should be bases seperated with comma. such as --bases A,T,C,G\n"
                    )
                    exit(-1)
        else:
...
def read_gTab_head(IN):
    ...
    rt_bd_cols = {
        file: (gTab_head["RT_" + file], gTab_head["BD_" + file]) for file in rt_files
    }
...
def calcRTReplicateCorrelation(
    IN, gTab_head, rt_bd_cols, files, bases=None, minBD=100, windowsize=100
):
    import scipy
    import scipy.stats
...
def plot_base_pie(base_ratio_list, files, baselist, outprefix):
    ...
    for i in range(len(files)):
        ...
        baseratiolabels = [
            "%s(%d%%)" % (b, 100 * r) for b, r in zip(baselist, base_ratio)
        ]
        plt.pie(
            base_ratio,
            labels=baseratiolabels,
            colors=colors,
            startangle=140,
            shadow=False,
            explode=explode,
...
def plot_cor_heatmap(file_cor, files, outfn):
    ...
    prect = pd.DataFrame(
        prect, columns=pure_file_name(files), index=pure_file_name(files)
    )
    mask = prect.copy()
    mask.loc[:, :] = False
...
file_cor, file_base, files = calcRTReplicateCorrelation(
    IN,
    gTab_head,
    rt_bd_cols,
    files,
...
heatmap_ploted = plot_cor_heatmap(
    file_cor, files, params["outFolder"] + "img/cor_heatmap.png"
)
