        ...
        if not line.startswith("@"):
            ...
            if len(data) != gTab_head["ColNum"]:
                sys.stderr.writelines(
                    "Error: actual column number (%s) != labeled number (%s)"
                    % (len(data), gTab_head["ColNum"])
                    + "\n"
                )
                exit(-1)
            IN.seek(last_pos)
...
def prepare_bedGraph(OUT, param, strand):
    ...
    head = (
        'track type=bedGraph name="%s" description="%s" color="%s" autoScale=off smoothingWindow=off graphType=bar'
...
def TrtCont_mode_To_bedGraph(IN, gTab_head, outdir, min_cov=0):
    ...
    prepare_bedGraph(
        SHAPE_PLUS, {"name": "TrtCont", "desc": "TrtCont", "color": "83,169,102"}, "+"
    )
    prepare_bedGraph(
        SHAPE_MINUS, {"name": "TrtCont", "desc": "TrtCont", "color": "83,169,102"}, "-"
    )
    prepare_bedGraph(
        SHAPE_VAR_PLUS,
        {"name": "TrtCont_var", "desc": "TrtCont_var", "color": "83,169,102"},
        "+",
    )
    prepare_bedGraph(
        SHAPE_VAR_MINUS,
    ...
    for line in IN:
        ...
        if D_BD >= min_cov and shape_Score != "-1":
            window_shape = [
                float(it) for it in data[window_i].strip(",").split(",") if it != "-1"
            ]
            ...
            if locus[1] == "+":
                SHAPE_PLUS.writelines(
                    "%s\t%s\t%s\t%.3f\n" % (locus[0], pos - 1, pos, float(shape_Score))
                )
                SHAPE_VAR_PLUS.writelines(
                    "%s\t%s\t%s\t%.3f\n" % (locus[0], pos - 1, pos, var)
                )
            elif locus[1] == "-":
                SHAPE_MINUS.writelines(
                    "%s\t%s\t%s\t%.3f\n" % (locus[0], pos - 1, pos, float(shape_Score))
                )
                SHAPE_VAR_MINUS.writelines(
                    "%s\t%s\t%s\t%.3f\n" % (locus[0], pos - 1, pos, var)
                )

        if D_BD > 20:
            if locus[1] == "+":
                D_BD_PLUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, D_BD)
                )
                N_RT_PLUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, N_RT)
                )
                D_RT_PLUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, D_RT)
                )
            elif locus[1] == "-":
                D_BD_MINUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, D_BD)
                )
                N_RT_MINUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, N_RT)
                )
                D_RT_MINUS.writelines(
                    "%s\t%s\t%s\t%s\n" % (locus[0], pos - 1, pos, D_RT)
                )

    SHAPE_PLUS.close()
    SHAPE_VAR_PLUS.close()
    SHAPE_MINUS.close()
    SHAPE_VAR_MINUS.close()
    D_BD_PLUS.close()
    N_RT_PLUS.close()
    D_RT_PLUS.close()
    D_BD_MINUS.close()
...
def Trt_mode_To_bedGraph(IN, gTab_head, outdir, min_cov=0):
    ...
    prepare_bedGraph(
        SHAPE_PLUS, {"name": "Trt", "desc": "Trt", "color": "202,75,78"}, "+"
    )
    prepare_bedGraph(
        SHAPE_MINUS, {"name": "Trt", "desc": "Trt", "color": "202,75,78"}, "-"
    )
    prepare_bedGraph(
        SHAPE_VAR_PLUS,
        {"name": "Trt_var", "desc": "Trt_var", "color": "202,75,78"},
        "+",
    )
    prepare_bedGraph(
        SHAPE_VAR_MINUS,
