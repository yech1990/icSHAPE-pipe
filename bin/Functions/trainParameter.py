...
def init():
    params = {
        ...
        "NAIFiles": None,
        "sizeFile": None,
        "outReport": "report.pdf",
        "debug": False,
        "subFac": (0.0, 0.1, 11),
        "Window": (100, 100, 5),
    }
    opts, args = getopt.getopt(
        sys.argv[1:], "hd:D:N:s:o:", ["debug", "subFac=", "Window="]
    )
...
def main():
    ...
    wSizes = construct_array(
        params["Window"][0], params["Window"][1], params["Window"][2]
    )
    subFacs = construct_array(
        params["subFac"][0], params["subFac"][1], params["subFac"][2]
    )
    assert subFacs[-1] <= 1.0

    record = init_rect(
        len(wSizes), len(subFacs), rowNames=wSizes, colNames=subFacs, init_value=0.0
    )
    ...
    for wSize in wSizes:
        for subFac in subFacs:
            sys.stdout.writelines("Start to train (%s, %s)..." % (wSize, subFac))
            sys.stdout.flush()
            ...
            genomeSHAPEToTransSHAPE(
                outGenomeSHAPE, params["sizeFile"], outTransSHAPE, params["debug"]
            )
    ...
    sns.heatmap(
        data=record, annot=True, fmt=".4f", cmap=sns.color_palette("coolwarm", 50)
    )
