...
def build_noscaffold_fasta(input_fa, output_fa, verbose=True):
    ...
    if verbose:
        sys.stderr.writelines(
            "Warning: Removed scaffolds: " + "\t".join(removed_chr_list) + "\n"
        )
...
def main():
    ...
    CMD = (
        "STAR --runMode genomeGenerate --genomeFastaFiles %s --genomeDir %s --runThreadN %s"
        % (params["inFile"], params["outDir"], params["threads"])
    )
    if params["gtfFile"]:
